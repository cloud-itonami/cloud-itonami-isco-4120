...
- `src/secretarial/store.kotoba` — `Store` protocol + `MemStore`:
  registered clients, committed records, an append-only audit ledger.
- `src/secretarial/advisor.kotoba` — `Advisor` protocol; `mock-advisor`
...
- `src/secretarial/governor.kotoba` — `SecretarialGovernor/check`: a
  ...
  sign-off.
- `src/secretarial/actor.kotoba` — `build-graph`, `run-request!`,
