(ns secretarial.actor
  "SecretarialActor — the ISCO-08 4120 independent secretarial actor
  as a `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one secretarial operation request
  ...
  The unconditional invariant: the SecretarialAdvisor can never
  directly commit a record or dispatch a robot action the
  SecretarialGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [secretarial.advisor :as advisor]
            [secretarial.governor :as governor]
            [secretarial.store :as store]))

(defn build-graph
  "Build a compiled SecretarialActor graph. `store` implements
  `secretarial.store/Store`. `advisor` implements
  `secretarial.advisor/Advisor` (defaults to `mock-advisor`).
