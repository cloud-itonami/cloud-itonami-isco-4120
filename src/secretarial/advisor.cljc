(ns secretarial.advisor
  "SecretarialAdvisor — proposes a secretarial operation (correspond
  on a client's behalf, file a document, disclose confidential
  correspondence, commit an employer to a contract without
  authorization) for a registered client. The advisor is swappable:
  ...
  and has no notion of client provenance or confidentiality/
  authorization risk; `secretarial.governor` is the independent
  system that decides whether the proposal may proceed, per the
  itonami actor pattern.

  A proposal is a map:
    {:op :correspond|:file|:disclose-confidential-correspondence|:commit-employer-to-contract
...
(def ^:private system-prompt
  "You are a secretarial advisor. Given a secretarial operation
